...
class GruNNChoosingBetsManager(NeuralNetworkManager):
    def __init__(self, train_set, val_set, should_hyper_tune, test_set):
        ...
        self.best_params = {
            'dropout_rate': 0.325,
            'gru_reccurent_regularization_factor': 0.0,
    ...
    def perform_model_learning(self, verbose=True):
        self.history = self.model.fit(x=[self.x_train[0], self.x_train[1], self.x_train[2]], y=self.y_train, epochs=2500,
                                      batch_size=512,
                                      ...
                                      callbacks=[
                                      # EarlyStopping(patience=50, monitor='val_loss', mode='min', min_delta=0.0005, verbose=1 if verbose is True else 0),
    ...
    def hyper_tune_model(self):
        tuner = kt.BayesianOptimization(self.create_model,
                                        ...
                                        overwrite=False)
        tuner.search(x=[self.x_train[0], self.x_train[1], self.x_train[2]], y=self.y_train, epochs=750, batch_size=512, verbose=2,
                     validation_batch_size=self.y_train.shape[0],
                     callbacks=[EarlyStopping(patience=50, monitor='val_loss', mode='min', verbose=1, min_delta=0.0005)],
                     validation_data=([self.x_val[0], self.x_val[1], self.x_val[2]], self.y_val))
